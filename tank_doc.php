...
<tr>
    ...
    <td>
    <?php  $sql2 = mysqli_query("SELECT * FROM fac_job_tank")     ?>
      				   <select name="txtservice_job_code">
                       <?php  while($row = mysqli_fetch_assoc($sql2) ):  ?>
                          <option value="<?= $row['serial_code']; ?>" <?php if ($row['id_tank'] == $_GET['id_tank'])echo ' selected="selected"'; ?> >
                            <?= $row['serial_code'];  ?>
                          </option>
                       <?php endwhile; ?>
                      </select>
...
<tr>
    ...
    <input type="hidden"  name="imageValue"  value="<?php echo $db->getFieldString('file'); ?>" />
  	...
  	<img src="http://www.solarprimeg.co.za/upload/upload_tank_doc/<?php if($db->getFieldString('image') != "") echo $db->getFieldString('image');else echo "default.jpg"?>" style="width: 100px; height: 100px;" >
    </td>
</tr>
</table>
